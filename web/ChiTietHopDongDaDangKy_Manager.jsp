...
<html>
    ...
    <div style="text-align: center; margin-left: 40px;">
        ...
        <p style="font-size: 23px;">${chuTro.name}</p>
    </div>
    <div style="text-align: center; margin-right: 40px;">
        <p style="font-size: 23px;">Bên B (Khách thuê):</p>
        <p style="font-size: 23px;">${khachThue.name}</p>
    </div>
</div>
            ...
            selector: '#contractContent',
                    ...
                    readonly: true, // Thiết lập chế độ chỉ đọc
    ...
    <body>
        <jsp:include page="adminUserManagement/sidebar.jsp"></jsp:include>
            ...
            <form action="ChiTietHopDongByAdmin" method="post">
                ...
                <textarea id="contractContent" name="contractContent" rows="4" style="width: 100%; padding: 10px; border: 1px solid #ccc; border-radius: 4px; resize: vertical; margin-bottom: 20px;"></textarea>

                <% if ("pending".equals(trangThai)) { %>
                <label for="reason" style="margin-top: 10px;">Lý do từ chối:</label>
                <textarea id="reason" name="reason" rows="2" placeholder="Nhập lý do từ chối ở đây..." style="width: 100%; padding: 10px; border: 1px solid #ccc; border-radius: 4px; resize: vertical; margin-bottom: 20px;"></textarea>
                <% } %>

                <div style="margin-top: 10px;">
                    <% if ("pending".equals(trangThai)) { %>
                    <!-- Button Gửi cho nút Chấp nhận -->
                    <button type="submit" name="action" value="accept" style="padding: 10px 20px; background-color: green; color: white; border: none; border-radius: 4px; cursor: pointer;">Chấp nhận</button>

                    <!-- Button Gửi cho nút Từ chối -->
                    <button type="submit" name="action" value="reject" style="padding: 10px 20px; background-color: red; color: white; border: none; border-radius: 4px; cursor: pointer;">Từ chối</button>
                    <% } %>

                    <!-- Button Xuất PDF sẽ không submit form -->
                    <button type="button" style="padding: 10px 20px; background-color: blue; color: white; border: none; border-radius: 4px; cursor: pointer;" onclick="exportToPdf()">Xuất PDF</button>
                </div>
            </form>
