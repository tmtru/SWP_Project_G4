 ...
 * @author Admin
 */
@WebServlet(name = "DanhSachKhachNoController", urlPatterns = {"/khach-no"})
public class DanhSachKhachNoController extends HttpServlet {
  private static final int ACCOUNTS_PER_PAGE = 10;
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        List<HoaDon> khachNo = hddao.getKhachNo(currentHouseId, searchTerm, start, ACCOUNTS_PER_PAGE);
        int total = hddao.getKhachNo(currentHouseId, searchTerm, null, ACCOUNTS_PER_PAGE).size();

        int totalPages = (int) Math.ceil((double) total / ACCOUNTS_PER_PAGE);
        request.setAttribute("khachNo", khachNo);
        request.setAttribute("currentPage", page);
        request.setAttribute("totalPages", totalPages);
        request.setAttribute("searchTerm", searchTerm);

        request.getRequestDispatcher("danh-sach-khach-no.jsp").forward(request, response);
    }
